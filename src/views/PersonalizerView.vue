<template>
  <div class="flex flex-col h-dvh overflow-hidden bg-app text-fore font-dm text-sm leading-[1.5]">
...
function updateCanvasSize() {
  ...
  if      (w >= 1600) { cvW.value = 396; cvH.value = 452 }
  else if (w >= 1280) { cvW.value = 350; cvH.value = 400 }
  else if (w >= 1024) { cvW.value = 316; cvH.value = 362 }
  else if (w >= 768)  { cvW.value = 294; cvH.value = 336 }
  else if (w >= 390)  { cvW.value = 232; cvH.value = 264 }
  else                { cvW.value = 200; cvH.value = 228 }
}
...
function onKeyDown(e) {
  ...
  if ((e.key === 'Delete' || e.key === 'Backspace') && selectedId.value && !isInput) {
    e.preventDefault(); removeEl(selectedId.value)
  }
