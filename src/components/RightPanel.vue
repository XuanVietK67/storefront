<template>
  ...
  >
    ...
    >
      <p class="font-syne text-[9px] font-bold tracking-[.12em] uppercase text-faint mb-[3px]">{{ t('panel.designTools') }}</p>
      <h2
    ...
    >
      <div class="flex items-end justify-between mb-3">
        <span class="text-[11px] font-syne tracking-wide uppercase text-faint">{{ t('panel.yourTotal') }}</span>
        <div class="text-right">
          ...
          >$34.99</span>
          <span class="block text-[10px] text-faint font-syne tracking-wide">{{ t('panel.plusPersonal') }}</span>
        </div>
      </div>
      <button
      ...
      >
        ...
        <span class="relative">{{ t('panel.addToCart') }}</span>
      </button>
    </div>
  </aside>
</template>
